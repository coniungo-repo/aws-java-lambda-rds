...
public class UserServiceImpl implements UserService {
    ...
    private final DatabaseService<User> dbService = new PgDataService<>();
    ...
    public Optional<UserDTO> getUserById(Long id, LambdaLogger logger) {
        ...
        User user = dbService.read(TABLE_NAME, "id", id, rs -> User.builder()
                .id(rs.getLong("id"))
    ...
    @Override
    public List<UserDTO> getAllUsers(int pageSize, int pageNumber, LambdaLogger logger) {
        logger.log("Service: Fetching paginated users");

        List<User> users = dbService.readPaginated(TABLE_NAME, pageSize, pageNumber, rs -> User.builder()
                .id(rs.getLong("id"))
                .username(rs.getString("username"))
                .email(rs.getString("email"))
                .build());
